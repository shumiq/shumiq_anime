...
describe('<Sync />', () => {
    ...
    it('should see sync buttons correctly', async () => {
        ...
        GooglePhotoApi.getAlbums.mockResolvedValue({
            albums: [
                { id: mockDatabase.animelist[0].gphotoid,title: '[Anime] ' + mockDatabase.animelist[0].title, mediaItemsCount: 2 },
                { id: mockDatabase.animelist[1].gphotoid,title: '[Anime] ' + mockDatabase.animelist[1].title + ' SS2', mediaItemsCount: 2 }
            ]
        });
        const wrapper = shallow(<Sync />);
        // When
        ...
        await flushPromises();
        // Then
        expect(wrapper.find('.row-anime').at(0).find('#btn-unsync')).toHaveLength(0);
        expect(wrapper.find('.row-anime').at(0).find('#btn-sync')).toHaveLength(0);
        expect(wrapper.find('.row-anime').at(1).find('#btn-unsync')).toHaveLength(0);
