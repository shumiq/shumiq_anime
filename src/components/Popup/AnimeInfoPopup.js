...
const AnimeInfoPopup = props => {
    ...
    const syncAnime = () => {
        ...
        state.studio = info.studios?.nodes[0]?.name ? info.studios?.nodes[0]?.name : 'none';
        ...
        state.genres = genres.filter(g => g.trim() !== '').sort().join(", ");
    ...
    return (
        <div className="AnimeInfoPopup">
            <Modal show={props.show} centered backdrop={true} keyboard={true} animation={true} onHide={closePopup}>
                        ...
                        <div id="infoPopup">
                            <div className="text-left">
                                ...
                                {info.nextAiringEpisode != null &&
                                    <p className="my-0">
                                        <b>Next Airing</b>: EP{info.nextAiringEpisode.episode} in {secondToDuration(info.nextAiringEpisode.timeUntilAiring)}
                                    </p>
                                }
                                <p className="my-0">
                                ...
                                {info.relations?.nodes &&
                                    <p>
                                        <b>Related Media</b>: {info.relations.nodes.map(related => related.title.userPreferred + ' (' + related.type + ')').join(', ')}
                                    </p>
                                }
                            </div>
                        </div>
                ...
                <Modal.Footer>
                    {IsAdmin() &&
                        <button type="button" className="btn btn-primary" onClick={syncAnime}>Sync</button>
                    }
                    {IsAdmin() &&
                        <button type="button" className="btn btn-primary" onClick={blackListResult}>Incorrect</button>
                    }
