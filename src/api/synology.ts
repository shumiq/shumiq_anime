...
const endPoint = `${process.env.REACT_APP_API_ENDPOINT?.toString()}/api/drive` || 'http//:localhost:3000/api/drive';
const downloadPath =
...
const SynologyApi = {
    ...
    list: async (path :string, sortByDate = false, isAdditional = false, isRetry = false): Promise<listResModel> => {
      let sid = localStorage.getItem("synology_sid");
      if(!sid || sid.length === 0) sid = await SynologyApi.signIn();
      const reqPath = `${path}${sortByDate ? sortBy.date : sortBy.name}${isAdditional ? additional : ''}&_sid=${sid}`;
      const response : {data : listResModel, status: string} = await axios.get(`${endPoint}/list?path=${encodeURIComponent(reqPath)}`);
