...
const GoogleDriveApi = {
    getFiles: async folderId => {
        ...
        let nextPageToken = '';
        while (true) {
            const response = await axios.get('https://www.googleapis.com/drive/v3/files?access_token=' + getAccessToken() + '&pageToken=' + nextPageToken + '&q=\'' + folderId + '\' in parents');
    ...
    createFolder: async (name, parentId) => {
        const response = await axios.post('https://www.googleapis.com/drive/v3/files?access_token=' + getAccessToken(), {
            "name": name,
    ...
    moveUploadFile: async (fileId, destinationId) => {
        await axios.patch('https://www.googleapis.com/drive/v3/files/' + fileId + '?access_token=' + getAccessToken() + '&addParents=' + destinationId + '&removeParents=' + uploadFolderId);
    }
}
